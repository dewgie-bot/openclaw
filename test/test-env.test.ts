...
describe("installTestEnv", () => {
  ...
  it("overlays an inherited external state dir without losing real-home auth fixtures", () => {
    const realHome = createTempHome();
    ...
    writeFile(path.join(inheritedStateDir, "credentials", "extra-token.txt"), "other\n");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".openclaw", "credentials", "extra-token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".openclaw", "agents", "main", "agent", "auth-profiles.json"),
