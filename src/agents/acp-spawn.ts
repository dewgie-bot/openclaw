...
export async function spawnAcpDirect(
  ...
  // Any thread-bound ACP spawn should keep bootstrap delivery in the bound thread,
  // regardless of whether the runtime mode is one-shot or persistent.
  const useInlineDelivery =
    hasExternalDeliveryTarget &&
    !effectiveStreamToParent &&
    (requestThreadBinding || (isNonThreadedRun && isNonThreadedAcpCompletionDeliveryEnabled(cfg)));
